/*  Wayne H Nixalo - 6-Oct-2016 05:08 - 16-Oct-2016 07:30
    Codingame: The Accountant
    Test08 Targeting I Algorithm   */
    ...
    struct TargetData {
        ...
        int closestXY[2];  // X,Y coords of closest enemy to player
    };

    /* prototypes */
    // void calcDrange(struct TargetData &TD, int dataCount, int enemyCount);    // calculates closest range of enemy to data point
    void calcErange(struct TargetData &TD, int enemyCount, int dataCount, int x, int y);    // calculates closest range of enemy to player
    ...
    int main()
    {
        ...
        while (1) {
            ...
            calcErange(TD, enemyCount, dataCount, x, y);

            /* Find Closest 2 threats to Evade */
            /* load first range */
            EThreat[0][0] = (int)TD.Erange;
            EThreat[0][1] = TD.closestXY[0];
            EThreat[0][2] = TD.closestXY[1];
            ...
            for (int i = 0; i < enemyCount; i++) {
                if (EThreat[0][0] > (int)sqrt((x - TD.xe[i])*(x - TD.xe[i])+(y - TD.ye[i])*(y - TD.ye[i]))) {
                    if (sqrt((x - TD.xe[0])*(x - TD.xe[0])+(y - TD.ye[0])*(y - TD.ye[0])) < 2400)
                        evadeCount += 1;
                    for (int j = 0; j < 3; j++) { EThreat[1][j] = EThreat[0][j]; }
            ...
            if (EVADE) {
                ...
                if (evadeCount < 2 && TD.Erange < 2300) {
                        Cmd = "MOVE ";
                        Cmd += to_string(2*x - EThreat[0][1]);
                        Cmd += ' ';
                        Cmd += to_string(2*y - EThreat[0][2]);
                        string s = " EVADE: ";
                        s += to_string(2*x - EThreat[0][1]);
                        s += ' ';
                        s += to_string(2*y - EThreat[0][2]);
                        Cmd += s;
                } else {
                    Cmd = "MOVE ";
    ...
    void calcErange(struct TargetData &TD, int enemyCount, int dataCount, int x, int y) {
        /* load first range */
        TD.Erange = sqrt((x - TD.xe[0])*(x - TD.xe[0])+(y - TD.ye[0])*(y - TD.ye[0]));
        TD.targetX = TD.xe[0];
        TD.targetY = TD.ye[0];
        TD.targetID = TD.id[0];
        TD.closestXY[0] = TD.targetX;
        TD.closestXY[1] = TD.targetY;
        /* scan through ranges of each enemy to player */
        for (int i = 0; i < enemyCount; i++) {
        ...
        /* find enemy ranges to DP -- if enemy closest to DP within tolerance range, Target */
        /* load first range */
        TD.Drange = sqrt((TD.dx[0] - TD.xe[0])*(TD.dx[0] - TD.xe[0])+(TD.dy[0] - TD.ye[0])*(TD.dy[0] - TD.ye[0]));
        /* scan through ranges of every enemy to every data point */
        for (int i = 0; i < dataCount; i++) {
            for (int k = 0; k < enemyCount; k++) {
                if (TD.Drange > sqrt((TD.dx[i] - TD.xe[k])*(TD.dx[i] - TD.xe[k])+(TD.dy[i] - TD.ye[k])*(TD.dy[i] - TD.ye[k]))) {
                    TD.Drange = sqrt((TD.dx[i] - TD.xe[k])*(TD.dx[i] - TD.xe[k])+(TD.dy[i] - TD.ye[k])*(TD.dy[i] - TD.ye[k]));
                    if (sqrt((x - TD.xe[k])*(x - TD.xe[k])+(y - TD.ye[k])*(y - TD.ye[k])) < (TD.Erange + 1500)) {
                        TD.targetID = TD.id[k];
                        TD.targetX = TD.xe[k];
                        TD.targetY = TD.ye[k];
                    }

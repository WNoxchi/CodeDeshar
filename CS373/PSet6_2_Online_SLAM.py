...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omega.value[0][0] = 1.; omega.value[1][1] = 1.
    ksi.value[0][0] = world_size/2.; ksi.value[1][0] = world_size/2.

    # data processing
    for k in range(len(data)):
        # n: index of robot pose: = 0
        n = 0
        ...
        for i in range(len(measurement)):
            # m: idnex of landmrk coord in matx/vec
        ...
        # expand matrix/vector
        # indices = [0,1] + range(4, dim+2) # Python2 only
        indices = [0,1]
        for i in range(4, dim+2): indices.append(i)
        omega = omega.expand(dim+2, dim+2, indices)
        ksi = ksi.expand(dim+2, 1, indices, [0])

        # update robot motion - diagonals
        for b in range(4):
        ...
        # factor our previous position
        indices = [col for col in range(2, len(omega.value))] # omega.value is now 2 longer than dim
        # indices = range(2, len(omega.value)) # Python2 only
...
# <full SLAM>
#
# Landmarks:  [[98.0, 93.0], [19.0, 94.0], [65.0, 56.0], [57.0, 68.0], [9.0, 9.0]]
# Robot: [x=71.58259 y=17.23868]
#
# Estimated Pose(s):
#     [49.999, 49.999]
#     [65.741, 61.729]
...
# <online SLAM>
#
# Landmarks:  [[76.0, 56.0], [39.0, 42.0], [71.0, 1.0], [67.0, 81.0], [14.0, 99.0]]
# Robot: [x=26.57684 y=19.14004]
#
# Estimated Pose(s):
#     [26.327, 18.583]
#
...
#     [66.704, 81.052]
#     [13.782, 100.530]
# Mu has correct dimensions.
# Omega has correct dimensions.
# Mu correct.
# Omega correct.
# Test case passed!

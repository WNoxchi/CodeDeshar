# CS373 - AI for Robotics - Udacity - Problem Set 3 - Particle Filters
# Wayne H Nixalo - 2017-Apr-20 13:19
...
class robot:
    ...
    def move(self, motion, tolerance = 0.001): # Do not change the name of this function
        ...
        # Equations:------------------------------
            # state: x, y, theta    ## Udacity Py shell not liking
            # turn angle = beta     ## non-ASCII Hellenic characters...
            # turn radius = R
            # steering angle = alfa
            # distance = d
            # length = L
            # turn circle center coords = (cx, cy)
            # ------------------------------------
            # beta = (d/L)*tan(alfa)
            # R = d/beta
            # cx = x - sin(theta)R
            # cy = y + cos(theta)R
            # x = cx + sin(theta+beta)R         <-- around circle
            # y = cy - cos(theta+beta)R
            # theta = (theta+beta) mod 2pi      ## self.orientation
            # x` = x + d*cos(theta)             <-- straight line
            # y` = y + d*sin(theta)
        ...
        result.distance_nosie = self.distance_noise
        # idk if you have to copy bearing noise for it to persist
        ...
        # beta = distance / length * tan(steering)
        beta = tan(steering) * distance / result.length
        ...
        # treat as turn
        elif abs(beta) >= tolerance:
            R = distance / beta
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            # result.orientation update comes before x,y updates
            result.orientation = (self.orientation + beta) % (2*pi)
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for z in range(len(landmarks)):
            dy, dx = landmarks[z][0] - self.y, landmarks[z][1] - self.x
            Z.append(atan2(dy, dx) - self.orientation)

            if add_noise:
                Z[z] += random.gauss(0.0, self.bearing_noise)

            Z[z] %= (2.0 * pi) # another way to fix angle wrapping

        # # I think this fixes an angle wrapping around beyond 2π

# WNixalo - 2017-Jun-19 01:42|19:07 - Udacity CS373 L6 Q6: Segmented CTE
# NOTE s:
# when u > 1: increment index
#     make sure it never goes beyond what's legal in path-length
#
# the CTE should be computed relative to the current index,
#     and is the sine-error using the DotΠ shown.
#
# finally: use spath; spath[index][0] = xi, spath[index][1] = yi
################################################################################
# -----------
# User Instructions
#
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        x1, y1 = spath[index][0], spath[index][1]
        x2, y2 = spath[index+1][0], spath[index+1][1]
        ...
        # how do I check that u never exceeds max size? what do if it does?

        if u > 1: index += 1

        # NOTE: !!! CTE must be RyDx - RxDy in numerator, not (+) !
        # NOTE: fuck yeah, got it. -- WNx
...
def twiddle(init_params):
    ...
    # WNx: adding a time out for twiddle operation:
    # timeout = 2000 # seems like 2000 is a bit too much... >:D
    ...
    while sum(dparams) > 0.0000001:
        for i in range(len(params)):
            ...
            err = 0
            for k in range(K):
                # NOTE: code error: https://discussions.udacity.com/t/bug-in-the-twiddle/59310
                # ret = main(grid, init, goal,
                #            steering_noise, distance_noise, measurement_noise,
...
# OUTPUT:
# [True, 0, 145]
# Correct! Your robot made it to the goal! Congratulations roboticist!
...
####### ####### ####### ####### ####### ####### ####### ####### ####### #######
# L6Q7: Fun with Parameters:

#twiddle([weight_data, weight_smooth, p_gain, d_gain])


# TESTED ON LOCAL MACHINE:
# Twiddle # 10 [0.10000000000000009, 0.19999999999999996, 2.0, 6.0]  ->  141.4
# Twiddle Loop No. 10

# Not quite as good as the default 145 step value. This was run for only 10
# iterations after 'fixing' the twiddle code's runtime errors.

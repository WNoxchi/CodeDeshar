# Wayne H Nixalo - 2017-Jun-21 13:19
# Udacity CS373 Robotic AI
# Lesson 6 - SLAM - Q22: Implementing SLAM
# ------------
# User Instructions
#
...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    # Add your code here!
    # <<NOTE: heavily following along with STAFF SOLN + my notes>>

    # get dimension of Ω matrix & ξ vector
    dim = 2 * (N + num_landmarks) # accts for x & y fields

    # initialize Ω & ξ: starting constraints = world_size / 2 = 50
    # NOTE: aha, you can index this matrix class via matrix.value[row][col]
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0], Omega.value[1][1] = 1., 1.
    ...
    # main loop to go thru all moves/updates
    for k in range(len(data)):
        # n is index of robot 'pose', (x0,y0),(x1,y1),etc..
        n = k * 2   # "path index is data-item * 2"

        # get measurement & motion data
        measurement = data[k][0]
        motion      = data[k][1]

        # `integrate the measurements` -- update Ω & ξ w/ measurement data
        for i in range(len(measurement)):   # go thru all the measurements
            # m is index of landmark fields
            # NOTE: oh ffs, measurement[i][0] = data[k][0][i][0] is just the
            #       index of the landmark measurement, called in make_data(..),
            #       computed in sense(..), & returned to make_data(..)
            #       The length of measurement, I think, is the number of
            #       landmarks the robot was able to see at that pose.

            # find index in matrix, of measurement: pathlen * measurement_indx
            m = 2 * (N + measurement[i][0])

            # update the information matrix/vector based on measurement data
            for b in range(2):      # `2` to acct for x,y
                # simple addition, with measurement_noise as inverse weight factor
                ...
                # in vector, add the measurement all divided by strength of noise
                ksi.value[n+b][0] += -measurement[i][1+b] / measurement_noise
                ksi.value[m+b][0] += measurement[i][1+b] / measurement_noise

        # update information matrix/vector based on robot motion
        # update diagonals
        for b in range(4):
            Omega.value[n+b][n+b] += 1. / motion_noise
        # update off-diagonals
        for b in range(2):      # `2` is the offset due to x,y
    ...
    # NOTE: that data structure (data) was a confusing axe-murderer.
    #       probably wouldn't have been so bad if I took the time to trace it
    #       out by hand. Kindof/sortof get this.
    #       Actually, it isn't hard. The killer is not knowing where what is in
    #       the datastrcture. Note to self: don't allow that to be a fail-point.
...
#     #
#     # Add your code here!
#     Omega = matrix([[0. for row in range(N+num_landmarks)] for col in range(N+num_landmarks)])
#     ksi   = matrix([[0.] for row in range(N+num_landmarks)])
#
#     # I don't think I can index into the matrix class, and I think I ought to
#     # make this as general as possible, ie: hardcode as little as possible:
#     initial_pos = 50.   # couldn't find in data
#     for step in range(len(data)):
#         # zero-lists to be indexed into and added as matrices
#         Omega_temp = [[0. for row in range(N+num_landmarks)] for  col in range(N+num_landmarks)]
#         ksi_temp = [[0.] for row in range(N+num_landmarks)]
#
#         # NOTE: do motn|meas noise divide postn|move values?
#
#         # 1st step: start position constraint
#         if step == 0:
#             Omega_temp[0][0] = 1.
#             ksi_temp[0][0] = initial_pos
#
#         # matrix index offset for landmarks = N
#         else:
#             Omega_temp[step][step], Omega_temp[N+step][step] = 1./motion_noise, -1./measurement_noise
#             Omega_temp[step][N+step], Omega_temp[N+step][N+step] = -1./measurement_noise, 1./motion_noise
#             ksi_temp[0][step], ksi_temp[N+step] = -data[step][0]/motion_noise, data[ste][0]/motion_noise
#
#         Omega += matrix(Omega_temp)
#         ksi   += matrix(ksi_temp)
#
#     mu = Omega.inverse() * ksi
#     #

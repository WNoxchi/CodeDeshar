# Wayne Nixalo - 2017-Jun-11 11:58
# CS373 AI for Robotics - Homework Assignment 5.4 - Racetrack Control
...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        #
        #
        # case 1: if on left turn of track:
        # cte = distance to center radius - radius; (deviation from track)
        if self.x < radius:
        	cte = sqrt((self.x - radius) ** 2 + (self.y - radius) ** 2) - radius
    	# case 2: if on right turn of track:
        elif self.x > 3. * radius:
        	cte = sqrt((self.x - 3. * radius) ** 2 + (self.y - radius) ** 2) - radius
        # case 3: if on top straight-away:
        elif self.y > radius:
        	cte = self.y - 2. * radius
        # case 4: if on bot straight away -- (-) bc moving opposite direction:
        else:
        	cte = -self.y

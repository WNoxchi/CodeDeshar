 ...
 * ---------------
 * This program handles the data management for an online music store
...
public class FlyTunesStore extends ConsoleProgram {
     ...
     *  of optinos. Returns the users selection. Note that
    ...
    private void listAlbums() {
        ...
        Iterator<String> albumIt = albums.keySet().iteratr();
    ...
    private int findSong(String name, String band) {
        ...
        for(int i = 0; i < songs.size(); i++) {
            if (songs.get(i).getSongName().equals(name)
                ...
                index = i;
                break;  // dont need to finish the loop
            }
        }
        return index;
    }

    /** Adds a new song to the stores inventory and returns that
     ...
     *  the inventory. he method may return null if the user
     *  decides not to enter a song (i.e., user just pressses
    ...
    private Song addSong() {
        ...
        itn songIndex = findSong(name, band);
        if (songIndex != -1) {
    ...
    /** Updates the price of a song in the stores inventory.
    ...
    private void updateSongPrice() {
        ...
        int songLIndex = findSong(name, band);
        if (songIndex == -1) {
